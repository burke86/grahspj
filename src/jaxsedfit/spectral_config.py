...
"""Configuration types for the reusable quasar spectral model."""
...
class InferenceConfig:
    ...
    parameters with different physical units on comparable scales.  The
    default structured mass matrix learns a compact dense block for each line
    complex, including that complex's ordered-width coordinates, while leaving
    shared width hyperparameters, continuum, host, and unrelated coordinates
    diagonal. The default 0.82 acceptance target is a conservative compromise
    between trajectory length and divergent-transition risk for these blocks.
    Set ``dense_mass=True`` only to
    request one fully dense metric, or ``line_block_dense_mass=False`` for a
    fully diagonal metric.  A separate warmup tree limit remains available,
    but by default warmup uses the same ceiling as retained draws.  For a hard
    tied-line fit, a useful explicit adaptation schedule is 500 warmup draws
    capped at depth 7 followed by retained draws capped at depth 8; the longer
    warmup is important, because lowering only the 250-draw warmup ceiling can
    leave the covariance estimate under-adapted.
    """
