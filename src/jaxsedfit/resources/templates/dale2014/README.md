# Dale 2014 template provenance

This directory contains vendored host-dust template files used by the `jaxsedfit` energy-balance implementation.

- Upstream project: `CIGALE`
- Upstream tag: `v2025.1`
- Upstream commit: `29cb909fe2636800b4acdb1dfc7129d8c8494a24`
- Upstream license: `CeCILL v2`
- Local license copy: [CeCILL-v2.txt](../../../../../LICENSES/CeCILL-v2.txt)

Current bundled files:

- `dhcal.dat`
- `spectra.0.00AGN.dat`
- `stellar_SED_age13Gyr_tau10Gyr.spec`

These files are used by the JAX host-dust loader in `jaxsedfit.preload`.
