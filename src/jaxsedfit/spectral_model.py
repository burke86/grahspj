    ...
    if n_f > 0:
        ...
        for complex_group in complex_groups:
            ...
            if bool(prior_config.get("standardize_active_priors", False)):
                ...
                probability = dist.Normal(0.0, 1.0).cdf(eps)
...
def _fe_template_component(
    ...
    *,
    ...
    ``fwhm_kms`` is the requested total width, while ``base_fwhm_kms`` is the
    intrinsic resolution of the empirical template. Requested widths below
    the template resolution are smoothly mapped to the minimum resolvable
    profile. Consequently, posterior values well below ``base_fwhm_kms`` are
    an unresolved upper limit rather than a measured physical width.
...
def _sample_distribution(prior_config, key, prior):
    ...
    if bool(prior_config.get("standardize_active_priors", False)):
        ...
        value = _standardized_prior_value(prior, eps)
        return numpyro.deterministic(key, value)
    return numpyro.sample(key, prior)


def _standardized_prior_value(prior, eps):
    """Transform a standard-normal coordinate into an exact prior draw."""
    # Avoid an inverse-CDF round trip for the two most common families.
    # Besides being cheaper, these analytic maps retain nonzero gradients in
    # the Normal tails; a clipped probability map becomes exactly flat.
    if isinstance(prior, dist.Normal):
        return prior.loc + prior.scale * eps
    if isinstance(prior, dist.LogNormal):
        return jnp.exp(prior.loc + prior.scale * eps)
    return prior.icdf(dist.Normal(0.0, 1.0).cdf(eps))
